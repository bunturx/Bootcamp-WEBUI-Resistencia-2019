...
class ItemInfo extends React.PureComponent {
  ...
  fetch(){
    fetch(
        `http://www.omdbapi.com/?i=${this.props.match.params.id}&plot=full&apikey=5242b7c7`
      )
        .then(item => item.json())
        .then(item => {
            console.log(item)
          this.setState({
            ...
            item.Poster.length > 10 ? this.setState({ poster: item.Poster }) : this.setState({ poster: "http://www.interlog.com/~tfs/images/posters/TFSMoviePosterUnavailable.jpg" })
        });
  }
  ...
  render() {
    ...
    return (
      <main className="itemInfo">
        <img className="itemPoster" src={this.state.poster} alt={`Poster ${this.state.title}`}/>
        <aside>
          <article>
